...
public class ShootProjectilePowerUpAction extends PowerUpAction {
    ...
    public static final PacketCodec<RegistryByteBuf, ShootProjectilePowerUpAction> PACKET_CODEC = PacketCodec.tuple(
            ...
            SoundEvent.ENTRY_PACKET_CODEC, (action -> action.sound),
            ShootProjectilePowerUpAction::new
    );

    private final RegistryEntry<EntityType<?>> projectile;
    private final RegistryEntry<SoundEvent> sound;
    //TODO: add shooting algorithm
    //TODO: add projectile NBT

    public ShootProjectilePowerUpAction(RegistryEntry<EntityType<?>> projectile, RegistryEntry<SoundEvent> sound) {
        this.projectile = projectile;
        this.sound = sound;
    }

    @Override
    protected PowerUpActionType<?> getType() {
    ...
    public void onTrigger(MinecraftServer server, ServerPlayerEntity player) {
        ...
        world.playSound(null, player.getX(), player.getY(), player.getZ(), this.sound, SoundCategory.NEUTRAL, 0.5F, 0.4F / (world.getRandom().nextFloat() * 0.4F + 0.8F));
