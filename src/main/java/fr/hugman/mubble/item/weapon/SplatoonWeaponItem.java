...
public class SplatoonWeaponItem extends Item {
        ...
        if (user instanceof PlayerEntity player) {
            if (weapon instanceof AutomaticShooterConfig config) {
                if (!player.getItemCooldownManager().isCoolingDown(stack)) {
                    this.shootShooterInkBullet(world, player, stack, config);
    ...
    private void shootShooterInkBullet(World world, PlayerEntity player, ItemStack stack, AutomaticShooterConfig config) {
        world.playSound(null, player.getX(), player.getY(), player.getZ(), MubbleSounds.SPLATTERSHOT_SHOOT, SoundCategory.PLAYERS, 0.5f, 1.0F);
        player.getItemCooldownManager().set(stack, (int) config.cooldown());
        if (!world.isClient) {
