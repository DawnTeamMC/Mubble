...
public class CapeFeatherItem extends Item {
    ...
    public ActionResult use(World worldIn, PlayerEntity playerIn, Hand handIn) {
        ...
        for (int i = 0; i < rand.nextInt(6) + 1; i++) {
            worldIn.addParticleClient(ParticleTypes.CLOUD, playerIn.getX() + (rand.nextInt(11) - 5) / 10F, playerIn.getY(), playerIn.getZ() + (rand.nextInt(11) - 5) / 10F, 0.0D, (rand.nextInt(3) + 1) / 10F, 0);
        }
