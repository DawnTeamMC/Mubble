...
public class EmptyBlock extends Block {
	...
	public void onEntityCollision(BlockState state, World worldIn, BlockPos pos, Entity entityIn) {
		if(!worldIn.isClient && entityIn.getVelocity().getY() > 0.0D) {
			...
			if(this == MubbleBlocks.SMB_EMPTY_BLOCK.getBlock()) {
				hitSound = MubbleSounds.BLOCK_EMPTY_BLOCK_HIT_SMB;
			}
			else if(this == MubbleBlocks.SMB3_EMPTY_BLOCK.getBlock()) {
				hitSound = MubbleSounds.BLOCK_EMPTY_BLOCK_HIT_SMB3;
			}
			else if(this == MubbleBlocks.SMW_EMPTY_BLOCK.getBlock()) {
				hitSound = MubbleSounds.BLOCK_EMPTY_BLOCK_HIT_SMW;
			}
			else if(this == MubbleBlocks.NSMBU_EMPTY_BLOCK.getBlock()) {
