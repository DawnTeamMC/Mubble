...
public class TallScarletTreeFeature extends AbstractTreeFeature<NoFeatureConfig>
{
	private static final IBlockState LOG = MubbleBlocks.SCARLET_LOG.getDefaultState();
	private static final IBlockState LEAVES = MubbleBlocks.SCARLET_LEAVES.getDefaultState();

	public TallScarletTreeFeature(boolean notify)
	{
		super(notify);
	}
	...
	private void crossSection(IWorld p_208529_1_, BlockPos p_208529_2_, float p_208529_3_)
	{
		...
		for(int j = -i; j <= i; ++j) for(int k = -i; k <= i; ++k) if (Math.pow((double)Math.abs(j) + 0.5D, 2.0D) + Math.pow((double)Math.abs(k) + 0.5D, 2.0D) <= (double)(p_208529_3_ * p_208529_3_))
		{
			BlockPos blockpos = p_208529_2_.add(j, 0, k);
			IBlockState iblockstate = p_208529_1_.getBlockState(blockpos);
			if (iblockstate.isAir(p_208529_1_, blockpos) || iblockstate.getMaterial() == Material.LEAVES) this.setBlockState(p_208529_1_, blockpos, LEAVES);
		}
	}
	...
	{
		if (y >= 0 && y < 5) return y != 0 && y != 4 ? 3.0F : 2.0F;
		else return -1.0F;
	}

	private void foliageCluster(IWorld p_202393_1_, BlockPos p_202393_2_)
	{
		for(int i = 0; i < 5; ++i) this.crossSection(p_202393_1_, p_202393_2_.up(i), this.foliageShape(i));
	}
	...
	{
		if (!p_208523_5_ && Objects.equals(p_208523_3_, p_208523_4_)) return -1;
		else
		{
			...
			for(int j = 0; j <= i; ++j)
			{
				...
				if (p_208523_5_) this.func_208520_a(p_208523_1_, p_208523_2_, blockpos1, LOG.with(BlockLog.AXIS, this.getLoxAxis(p_208523_3_, blockpos1)));
				else if (!this.canGrowInto(p_208523_2_, blockpos1)) return j;
			}
			return -1;
		}
	}
	...
	private EnumFacing.Axis getLoxAxis(BlockPos p_197170_1_, BlockPos p_197170_2_)
	{
		...
		if (k > 0)
		{
			if (i == k) enumfacing$axis = EnumFacing.Axis.X;
			else if (j == k) enumfacing$axis = EnumFacing.Axis.Z;
		}
		return enumfacing$axis;
	}

	private void makeFoliage(IWorld p_208525_1_, int p_208525_2_, BlockPos p_208525_3_, List<TallScarletTreeFeature.FoliageCoordinates> p_208525_4_)
	{
		for(TallScarletTreeFeature.FoliageCoordinates bigtreefeature$foliagecoordinates : p_208525_4_) if (this.trimBranches(p_208525_2_, bigtreefeature$foliagecoordinates.getBranchBase() - p_208525_3_.getY())) this.foliageCluster(p_208525_1_, bigtreefeature$foliagecoordinates);
	}
	...
	private void makeBranches(Set<BlockPos> p_208524_1_, IWorld p_208524_2_, int p_208524_3_, BlockPos p_208524_4_, List<TallScarletTreeFeature.FoliageCoordinates> p_208524_5_)
	{
		for(TallScarletTreeFeature.FoliageCoordinates bigtreefeature$foliagecoordinates : p_208524_5_)
		{
	...
	public boolean place(Set<BlockPos> changedBlocks, IWorld worldIn, Random rand, BlockPos position)
	{
		...
		else
		{
			...
			int i1 = i - 5;
			List<TallScarletTreeFeature.FoliageCoordinates> list = Lists.newArrayList();
			list.add(new TallScarletTreeFeature.FoliageCoordinates(position.up(i1), l));

			for(; i1 >= 0; --i1)
			{
				float f = this.treeShape(i, i1);
				if (!(f < 0.0F)) for(int j1 = 0; j1 < k; ++j1)
				{
					...
					if (this.makeLimb(changedBlocks, worldIn, blockpos, blockpos1, false) == -1)
					{
						...
						if (this.makeLimb(changedBlocks, worldIn, blockpos2, blockpos, false) == -1) list.add(new TallScarletTreeFeature.FoliageCoordinates(blockpos, blockpos2.getY()));
					}
				}
			}
	...
	{
		if (!p_208528_2_.getBlockState(p_208528_3_.down()).canSustainPlant(p_208528_2_, p_208528_3_.down(), net.minecraft.util.EnumFacing.UP, ((BlockSapling)MubbleBlocks.SCARLET_SAPLING))) return -1;
		else
		{
			int i = this.makeLimb(p_208528_1_, p_208528_2_, p_208528_3_, p_208528_3_.up(p_208528_4_ - 1), false);
			if (i == -1) return p_208528_4_;
			else return i < 6 ? -1 : i;
		}
	}
