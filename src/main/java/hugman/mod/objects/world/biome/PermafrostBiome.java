...
public class PermafrostBiome extends Biome
{
	...
	{
        super((new Biome.BiomeBuilder()).surfaceBuilder(new CompositeSurfaceBuilder<>(MubbleWorld.SurfaceBuilder.PERMAFROST_SURFACE_BUILDER, MubbleWorld.SurfaceBuilder.PERMAROCK_SURFACE)).precipitation(Biome.RainType.NONE).category(Biome.Category.NETHER).depth(0.1F).scale(0.2F).temperature(0.0F).downfall(0.0F).waterColor(4159204).waterFogColor(329011).parent((String)null));
        this.addStructure(Feature.FORTRESS, new FortressConfig());
        this.addCarver(GenerationStage.Carving.AIR, createWorldCarverWrapper(MubbleWorld.Carvers.PERMAFROST_CAVE_WORLD_CARVER, new ProbabilityConfig(0.2F)));
        ...
        this.addFeature(GenerationStage.Decoration.UNDERGROUND_DECORATION, createCompositeFeature(Feature.MINABLE, new MinableConfig(BlockMatcher.forBlock(MubbleBlocks.PERMAROCK), MubbleBlocks.PERMAFROST_BISMUTH_ORE.getDefaultState(), 6), COUNT_RANGE, new CountRangeConfig(10, 10, 20, 128)));
        this.addFeature(GenerationStage.Decoration.UNDERGROUND_DECORATION, createCompositeFeature(Feature.MINABLE, new MinableConfig(BlockMatcher.forBlock(MubbleBlocks.PERMAROCK), Blocks.ICE.getDefaultState(), 33), NETHER_MAGMA, new FrequencyConfig(4)));
