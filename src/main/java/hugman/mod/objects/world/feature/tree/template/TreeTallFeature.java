...
public class TreeTallFeature extends AbstractTreeFeature<NoFeatureConfig>
{
	private final IBlockState LOG;
	private final IBlockState LEAVES;
	private final IPlantable SAPLING;

	public TreeTallFeature(boolean notify, Block log, Block leaves, Block sapling)
	{
		super(notify);
	...
	public boolean place(Set<BlockPos> changedBlocks, IWorld worldIn, Random rand, BlockPos position)
	{
		...
		else
		{
			...
			int i1 = i - 5;
			List<TreeTallFeature.FoliageCoordinates> list = Lists.newArrayList();
			list.add(new TreeTallFeature.FoliageCoordinates(position.up(i1), l));
			...
			for(; i1 >= 0; --i1)
			{
				...
				if (!(f < 0.0F)) for(int j1 = 0; j1 < k; ++j1)
				{
					...
					if (this.makeLimb(changedBlocks, worldIn, blockpos, blockpos1, false) == -1)
					{
						...
						if (this.makeLimb(changedBlocks, worldIn, blockpos2, blockpos, false) == -1) list.add(new TreeTallFeature.FoliageCoordinates(blockpos, blockpos2.getY()));
					}
				}
			}
	...
	private void crossSection(IWorld p_208529_1_, BlockPos p_208529_2_, float p_208529_3_)
	{
		...
		for(int j = -i; j <= i; ++j) for(int k = -i; k <= i; ++k) if (Math.pow((double)Math.abs(j) + 0.5D, 2.0D) + Math.pow((double)Math.abs(k) + 0.5D, 2.0D) <= (double)(p_208529_3_ * p_208529_3_))
		{
			...
			if (iblockstate.isAir(p_208529_1_, blockpos) || iblockstate.isIn(BlockTags.LEAVES)) this.setBlockState(p_208529_1_, blockpos, LEAVES);
		}
	}
	...
	private int makeLimb(Set<BlockPos> p_208523_1_, IWorld p_208523_2_, BlockPos p_208523_3_, BlockPos p_208523_4_, boolean p_208523_5_)
	{
		...
		else
		{
			...
			for(int j = 0; j <= i; ++j)
			{
				...
				if (p_208523_5_) this.func_208520_a(p_208523_1_, p_208523_2_, blockpos1, LOG.with(BlockLog.AXIS, this.getLogAxis(p_208523_3_, blockpos1)));
	...
	private EnumFacing.Axis getLogAxis(BlockPos p_197170_1_, BlockPos p_197170_2_)
	{
	...
	private void makeFoliage(IWorld p_208525_1_, int p_208525_2_, BlockPos p_208525_3_, List<TreeTallFeature.FoliageCoordinates> p_208525_4_)
	{
		for(TreeTallFeature.FoliageCoordinates bigtreefeature$foliagecoordinates : p_208525_4_) if (this.trimBranches(p_208525_2_, bigtreefeature$foliagecoordinates.getBranchBase() - p_208525_3_.getY())) this.foliageCluster(p_208525_1_, bigtreefeature$foliagecoordinates);
	}
	...
	private void makeBranches(Set<BlockPos> p_208524_1_, IWorld p_208524_2_, int p_208524_3_, BlockPos p_208524_4_, List<TreeTallFeature.FoliageCoordinates> p_208524_5_)
	{
		for(TreeTallFeature.FoliageCoordinates bigtreefeature$foliagecoordinates : p_208524_5_)
		{
	...
	{
		if (!p_208528_2_.getBlockState(p_208528_3_.down()).canSustainPlant(p_208528_2_, p_208528_3_.down(), net.minecraft.util.EnumFacing.UP, (SAPLING))) return -1;
		else
		{
