...
public final class VoyageWorlds {
    private static FantasyVoyageWorldProvider current;

    private VoyageWorlds() {
    }

    public static void register() {
        ServerLifecycleEvents.SERVER_STARTED.register(server -> current = new FantasyVoyageWorldProvider(server));
    ...
    public static VoyageWorldProvider get(MinecraftServer server) {
        FantasyVoyageWorldProvider provider = current;
        if (provider == null) {
