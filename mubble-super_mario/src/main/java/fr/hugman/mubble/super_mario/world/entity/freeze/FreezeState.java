...
public record FreezeState(long startedAt, long endsAt) {
    ...
    /** An {@link #endsAt} that never arrives, for a freeze only a command or fire can end. */
    ...
    public static FreezeState endless(long gameTime) {
        return new FreezeState(gameTime, NEVER);
    }

    public boolean isEndless() {
        return this.endsAt == NEVER;
    }
    ...
    /**
     * @return how much longer the entity stays frozen, in ticks, never below zero, and
     * {@link Integer#MAX_VALUE} for an endless freeze
     */
    ...
    public boolean hasExpired(long gameTime) {
        return !this.isEndless() && gameTime >= this.endsAt;
    }

    /**
     * @return the same freeze, cut short by {@code ticks}, never ending before it started. An
     * endless freeze is returned untouched: nothing chips away at one, it is called off or it lasts.
     */
